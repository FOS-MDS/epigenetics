---
title: "Epigenomics Practical 2021"
...
`` `r emo::ji("warning")` `` *You can use `by` to calculate a mean and sd of groups*
...
`` `r emo::ji("warning")` `` *Have a look at `geom_histogram`*
...
`` `r emo::ji("warning")` `` *Have a look at `geom_violin` and geom_boxplot. Put the annotation on the x-axis and the DNA methylation level on the y-axis*
...
`` `r emo::ji("warning")` `` *You can add a secondary group by adding that group as fill colour!*
...
`` `r emo::ji("warning")` `` *Have a look at the `table` function*
